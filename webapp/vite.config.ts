...
import { feishuProxy } from "./build/feishu-proxy-plugin";
...
export default defineConfig(async ({ mode }) => {
  ...
  return {
    server: {
      // start-local.cmd sets TPV_AUTO_OPEN=1 so the browser opens automatically
      // when launched by double-clicking the bat file.
      open: process.env.TPV_AUTO_OPEN === "1",
